...
public class CommentsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetByStory(int storyId, [FromQuery] int page = 1, [FromQuery] int pageSize = 10)
    {
        try
        {
            ...
            { 
                status = true, 
                message = "L?y danh s�ch b�nh lu?n truy?n th�nh c�ng.", 
                data = new { comments, total = count, page, pageSize }
            });
        }
        catch (KeyNotFoundException)
        {
            return NotFound(new { status = false, message = "Kh�ng t�m th?y truy?n.", data = (object?)null });
        }
    }
    ...
    public async Task<IActionResult> GetByChapter(int chapterId, [FromQuery] int page = 1, [FromQuery] int pageSize = 10)
    {
        try
        {
            ...
            { 
                status = true, 
                message = "L?y danh s�ch b�nh lu?n ch??ng th�nh c�ng.", 
                data = new { comments, total = count, page, pageSize }
            });
        }
        catch (KeyNotFoundException)
        {
            return NotFound(new { status = false, message = "Kh�ng t�m th?y ch??ng.", data = (object?)null });
        }
    }
    ...
    public async Task<IActionResult> CreateForChapter(int chapterId, [FromBody] CommentCreateDTO dto)
    {
        ...
        try
        {
            ...
            return StatusCode(201, new { status = true, message = "T?o b�nh lu?n th�nh c�ng.", data = created });
        }
    ...
    public async Task<IActionResult> Update(int id, [FromBody] CommentUpdateDTO dto)
    {
        ...
        try
        {
            ...
            return Ok(new { status = true, message = "C?p nh?t b�nh lu?n th�nh c�ng.", data = updated });
        }
        catch (ArgumentException ex)
        {
            return BadRequest(new { status = false, message = ex.Message, data = (object?)null });
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        try
        {
            ...
            return ok
                ? Ok(new { status = true, message = "X�a b�nh lu?n th�nh c�ng.", data = new { commentId = id } })
                : NotFound(new { status = false, message = "Kh�ng t�m th?y b�nh lu?n ?? x�a.", data = (object?)null });
        }
        catch (KeyNotFoundException ex)
        {
            return NotFound(new { status = false, message = ex.Message, data = (object?)null });
